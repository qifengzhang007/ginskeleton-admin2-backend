...
func InitApiRouter() *gin.Engine {
	...
	//router.GET("/index_show", (&api.Index{}).IndexShow)

	//处理静态资源（不建议gin框架处理静态资源，参见 Public/readme.md 说明 ）
	router.Static("/public", "./public")
	router.Static("/storage/img", "./storage/img")   //  定义静态资源路由与实际目录映射关系
	router.StaticFile("/abcd", "./public/readme.md") // 可以根据文件名绑定需要返回的文件名

	//  创建一个门户类接口路由组
	vApi := router.Group("/api/v1/portal/")
	{
		// 创建一个websocket,如果ws需要账号密码登录才能使用，就写在需要鉴权的分组，这里暂定是开放式的，不需要严格鉴权，我们简单验证一下token值
		vApi.GET("ws", validatorFactory.Create(consts.ValidatorPrefix+"WebsocketConnect"))
