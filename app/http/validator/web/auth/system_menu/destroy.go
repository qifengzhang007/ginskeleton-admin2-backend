...
func (s SystemMenuDestroy) CheckParams(context *gin.Context) {
	//1.基本的验证规则没有通过
	if err := context.ShouldBind(&s); err != nil {
		errs := gin.H{
			"tips": "SystemMenuDestroy  参数校验失败，参数不符合规定, id ≥ 1",
			"err":  err.Error(),
		}
	...
	if extraAddBindDataContext == nil {
		response.ErrorSystem(context, "SystemMenuDestroy 表单验证器json化失败", "")
	} else {
		// 验证完成，调用控制器,并将验证器成员(字段)递给控制器，保持上下文数据一致性
		(&auth.SystemMenuController{}).Destroy(extraAddBindDataContext)
	}
}
