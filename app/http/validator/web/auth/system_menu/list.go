...
func (s SystemMenuList) CheckParams(context *gin.Context) {
	//1.基本的验证规则没有通过
	if err := context.ShouldBind(&s); err != nil {
		errs := gin.H{
			"tips": "SystemMenuList  参数校验失败，参数不符合规定, page ≥ 1、limits ≥ 1、fid  ≥ 1 、title 可空",
			"err":  err.Error(),
		}
	...
	if extraAddBindDataContext == nil {
		...
		(&auth.SystemMenuController{}).List(extraAddBindDataContext)
	}
}
