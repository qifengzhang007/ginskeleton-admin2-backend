...
type OrgPostList struct {
	Fid
	Title string `form:"title" json:"title" binding:"min=0"` // 必填，页面值>0
...
func (a OrgPostList) CheckParams(context *gin.Context) {
	...
	if extraAddBindDataContext == nil {
		response.ErrorSystem(context, "AuthOrganizationPostList表单验证器json化失败", "")
	} else {
		// 验证完成，调用控制器,并将验证器成员(字段)递给控制器，保持上下文数据一致性
		(&auth.OrganizationPostController{}).List(extraAddBindDataContext)
	}
}
